...
identify the finite-volume replicon susceptibility.  Our proof does not use the identification of
the limiting free energy with the Parisi variational formula, although it is inspired by the proof of   Parisi formula in [Talagrand, Ann. Math., 2006].
\end{abstract}
...
In the present paper, we consider the Sherrington--Kirkpatrick model, and in particular the replica symmetry in the de Almeida-Touless region.
...
The existence and uniqueness for all $h>0$ are obtained by a standard way (see, for example, \cite[Proposition~1.3.8]{Talagrand}).
The continuity of $q(\beta ,h )$ in $(\beta , h)$ follows directly from the uniqueness.
...
These ideas have been used far beyond condensed matter physics. In computer science, random constraint satisfaction problems such as random $K$-SAT and random graph coloring show threshold phenomena similar to replica symmetry breaking, and algorithms based on the cavity method, such as belief and survey propagation, are still among the best known methods for these problems (see \cite{MezardParisiZecchina2002,MezardMontanari2009}). In the information theory and the high-dimensional statistics, the free energy of a suitable spin glass model gives the fundamental limits in error-correcting codes, compressed sensing, and inference in graphical models (see \cite{Nishimori2001,ZdeborovaKrzakala2016}). In machine learning, the Hopfield model of associative memory is also a spin glass model in which the stored patterns play the role of quenched disorder (see \cite{Hopfield1982}), and spin glass methods have been used to study the loss landscapes and training dynamics of neural networks. More generally, the notions of frustration, energy landscapes, and metastability, identified in spin glasses, are now standard tools in the study of complex systems, from protein folding to financial networks.

The rigorous mathematical study of the SK model developed along different routes. An early landmark was the work by Aizenman, Lebowitz, and Ruelle \cite{AizenmanLebowitzRuelle1987}, who proved Gaussian fluctuations of the free energy at high temperature and zero external field by using a cluster expansion. At zero external field, replica symmetry is now known to hold for all $\beta<1$, and the critical point $\beta=1$ separates the replica-symmetric and symmetry-breaking regimes. With an external field, finding the full replica-symmetric region turned out to be much harder. Talagrand  proved the replica-symmetric formula at sufficiently high temperature by the cavity method, and a simple interpolation argument due to Lata\l{}a gave another proof in the range $\beta \in [0,1/2)$ and $h\in \R$. A recent work of the authors extended Lata\l{}a's argument to a wider range including the region $\beta\leq 1$ (see \cite{KusuokaNakajima1}).

In parallel, a different line of works dealt with the limiting free energy at all temperatures. In \cite{Guerra} Guerra introduced an interpolation argument that proved the appropriate Parisi bound, and then in \cite{TalagrandParisi} Talagrand proved the matching opposite bound. Together, these results established the Parisi formula for the SK model. This turned the study of the phase diagram of the free energy into a rigorous variational problem. In particular, the replica symmetry can be characterized through the minimizer of the Parisi functional, and this variational point of view has become one of the main tools for studying the AT transition.

One question remained: does the AT stability condition exactly characterize the replica-symmetric phase of the SK model with a nonzero external field? Several rigorous results proved the replica symmetry in regions inside the predicted AT region, or the replica symmetry breaking on the other side of the line, but reaching the physical boundary itself remained difficult and had been a major open problem in this field. Very recently, in \cite{Lopatto} Lopatto proved that the replica symmetry holds throughout the AT region \eqref{eq: AT region} 
 using the variational characterization of the Parisi formula. 
...
We briefly describe the schetch of the proof of the first claim in the main result.
The second and third are obtained from this using a standard interpolation argument (see Section~\ref{sec:conclusion}).
...
corresponding one-step replica-symmetry-breaking formula. Using a similar argument to Lopatto's work \cite{Lopatto}, one obtains the flatness condition that for $u>0$ small enough,
 \begin{equation}
\mathfrak P_s(u )
= \mathfrak P_s(0). %+ o(u ).
...
\end{equation*} 
By some computationis with Gaussian integration by parts, we have for $u_0>0$ sufficiently small,  
\[
...
\exp(-c_{\mathcal K,\delta}N).
\end{align*}
Therefore, we reach a weak concentration for overlap:
\begin{equation}
\begin{split}
    \nu_s\left(
|Q_{12}|\ge\delta
...
the cavity bootstrap developed in the high-temperature analysis by Talagrand (see \cite[Chapter~1]{Talagrand}).
...
The rest of the paper is the proof of Theorem~\ref{thm:main}.
In Section~\ref{sec:coercivity} we give estimates on the coercivity of the replica-symmetry.
The argument in this part is inspired by the approach by Talagrand given in \cite{TalagrandParisi}, and including a lot of technical and explicit calculations.
Section~\ref{sec:concentration} gives estimates about the overlap concentration. This part is similar to the argument in \cite{KusuokaNakajima1}.
...
In Appendix, we present the proofs of some technical results which are applied in Sections~\ref{sec:coercivity}~and~\ref{sec:concentration}.
In Section~\ref{app:cavity-identities} we give the proof of Proposition~\ref{prop:cavity}, which is about the estimates of overlaps in Section~\ref{sec:concentration}.
...
In Section~\ref{sec:Price} we give Price's theorem, which is applied also in Section~\ref{subsec:GT}.
...
\end{equation}
Indeed, suppose that $h\ge\sigma^2$. It is straightforward to check that 
\[
 1-q
 =
 \E\sech^2(h+\sigma Z)
...
\[
 (1-q)(1+\beta^2q)\le1.
\]
Organizing this with $q>0$ implies $\beta^2(1-q)\le1$. This contradicts the assumption 
...
For each $v$, the breaking points and levels are given as follows.

\begin{center}
\begin{tabular}{|c||c|c|}
\hline
& the breaking points $(a_j)_{j=0}^M$ & the levels $(m_j)_{j=0}^{M}$ \\[1mm]
\hline
$|v|=0$ & $0,q,1$ & $0,0,1$\\[1mm]
\hline
...
In the case that $0\le v\le q$, put $t=s\beta^2(q-v)$, let
...
Now we consider the case that $q<v\le1$. Let $t \coloneqq s\beta^2(v-q)$.
Similarly to the case that $-q\le v\le q$, it holds that for $q< v< 1$
\begin{align*}
 U_s^{0,v}(v,x_1,x_2)
...
\end{align*}
Note that these equalities holds also for $v=1$.
...
We also need a strict gap in the case that $v<-q$.  Suppose first that $s>0$ and let $t=s\beta^2(|v|-q)>0$.
Again, similarly to the case that $-q\le v\le q$, it holds that for $v< -q$
\[
 U_s^{0,v}(|v|,x_1,x_2)
 =s\beta^2(1-|v|)+\log\cosh x_1+\log\cosh x_2.
\]
Hence, noting that $|v|>q$, in a similar way to above we have
\[
 U_s^{0,v}(q,x_1,x_2)
 =
 s\beta^2(1-|v|)
...
\end{equation}
In the case that $0\le v<q$ and $q<v\le1$, this follows from  \eqref{eq:ATsign} and \eqref{eq:GT-zero-positive-simple}  by moving
...
For the case that $v<-q$ and $s=0$, \eqref{eq:GT-szero-simple} gives the claim by taking a sufficiently small negative $\lambda\in [-1,0)$.
...
bound above makes the right-hand side at most $C_{\cK}/N$.  This proves \eqref{eq:main2}.
...
%
&= 2 N \log 2 + \sum _{i=1}^N \log f_\lambda \left( Y_{0,i} + \sum _{j=1}^M z_{i,j}^1 , Y_{0,i} + \sum _{j=1}^M z_{i,j}^2 \right) -\lambda N v .
\end{align*}
...
\]
This inequality and the independance of $Y_{0,i}$ in $i$ yield
\begin{equation}
\varphi_N(0)
\leq
...
For each $t\in {\mathbb R}$, let $X(t)$ be an $n$-dimensional centered Gaussian vector with a covariance matrix $C(t) = (C_{ij}(t))_{i,j =1}^n$.
...
\begin{proof}
It is sufficient to show the claim for $f\in C_b^2({\mathbb R}^n)$ with a compact support. 
Let ${\mathcal F}g$ be a Fourier transformation of $g$, i.e.
\[
...
\end{align*}
where $\langle \cdot ,\cdot \rangle$ is the innter product on ${\mathbb R}^n$ and $\det C^\varepsilon(t)$ is the determinant of $C^\varepsilon(t)$.
Since, as $\varepsilon \downarrow 0$, $X^\varepsilon (t)$ converges to $X(t)$ in law and $ C^\varepsilon (t)$ converges to $C(t)$ component-wise, the differentiation with respect $t$ of both sides after taking limit as $\varepsilon \downarrow 0$ yields
\begin{align*}
