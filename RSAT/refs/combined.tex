...
The present paper provides an alternative proof of the replica-symmetric free energy formula in the de Almeida--Thouless region, which was recently established by Lopatto [arXiv:2604.11921]. Moreover, we prove the central limit theorem for the overlap in this region. 
...
\begin{theorem}
\label{thm:overlap-clt-intro}
For every fixed $(\beta,h)\in\cK$, the centered and rescaled overlap
converges weakly with respect to $\nu_1$\footnote{\textcolor{red}{Here $\nu_1=\mathbb E\langle\,\cdot\,\rangle_1$ defines the disorder-averaged two-replica law.  The Lean theorem proves weak convergence of this averaged probability measure; it does not assert almost-sure weak convergence of the random Gibbs law for fixed disorder.}} as
\[
 \sqrt N\,(R_{12}-q)
...
It is worth noting that the estimate above was previously known in a sufficiently high-temperature regime from Talagrand's cavity analysis and from an independent unpublished interpolation argument of Lata{\l}a; see \cite{Talagrand,KusuokaNakajima1}. In \cite{KusuokaNakajima1}, we recently refined Lata{\l}a's argument and substantially enlarged the range of parameters for which such a quantitative estimate holds. The main contribution of the present work is to extend this estimate to the entire strict de Almeida--Thouless region. This is expected to be the maximal open region in which replica-symmetric overlap fluctuations remain on the $N^{-1/2}$ scale. Establishing replica symmetry up to the de Almeida--Thouless line had remained an open problem for many years. Very recently, Lopatto \cite{Lopatto} resolved this problem by proving that the Parisi minimizer is replica symmetric throughout this region, hence the Parisi formula coincides with the replica-symmetric formula. His result builds on Talagrand's identification of the limiting free energy with the Parisi variational formula \cite{TalagrandParisi}.

The fluctuations of the overlap have also been studied extensively. In zero external field, for $\beta<1$, Comets and Neveu proved a central limit theorem for the overlap; see \cite{CometsNeveu}. At the critical point $(\beta,h)=(1,0)$, Du and Huang very recently identified the critical $N^{-1/3}$ scale and determined the limiting quenched distribution of the rescaled overlap \cite{DuHuangCriticalOverlap}. For nonzero external field, Gaussian fluctuations of the centered overlaps on the $N^{-1/2}$ scale were established, in a sufficiently high-temperature region, independently by Talagrand and by Guerra and Toninelli \cite{TalagrandRSB,GuerraToninelliCLT}.

Our results provide a quantitative finite-volume counterpart to the identification of the replica-symmetric phase and, at the same time, extend the fluctuation theory to the full strict de Almeida--Thouless region. More precisely, rather than asking only whether the thermodynamic limit is replica symmetric, we  prove the quantitative estimate stated in Theorem~\ref{thm:main}. Moreover, we prove a central limit theorem for the overlap. Thus, our results give a finite-size and fluctuation-level description of the replica-symmetric phase throughout the strict de Almeida--Thouless region.
...
For each $v$, the  breakpoints  and levels are given as follows.\footnote{\textcolor{red}{Correction: an earlier version listed only $(m_j)_{j=1}^M$ although the heading specifies $(m_j)_{j=0}^M$.  The displayed rows now include $m_0$; for example, the full level vector for $|v|=0$ is $(0,0,1)$.}}
...
\]
we have\footnote{\textcolor{red}{Correction: an earlier version missed the factor $2$ in this $q<v$ Guerra--Talagrand recursion.  Since $m_{M-1}=1/2$, $\mathcal T_{1/2,C}F=2\log\E e^{F/2}$.  The factor $2$ displayed below is required to obtain $2\log\cosh x+s\beta^2(1-q)$ when $x_1=x_2=x$.}}
\begin{align*}
 U_s^{0,v}(q,x_1 ,x_2 )
 &=s\beta^2(1-v)
...
$a_{\cK}\in (0,1)$ such that\footnote{\textcolor{red}{Correction of a logical gap in an earlier version: the sign of $g_s(v)-v$ alone gives only a pointwise gap and does not imply a uniform quadratic gap as $v\to q$.  The local slope estimate proved above is needed for the displayed bound.}}
\begin{equation}
 (g_s(v)-v)(v-q)
 \le-a_{\cK}(v-q)^2,
...
Write $\sigma=(\rho,\varepsilon)$ and, for each replica $a$, set
\[
...
\begin{equation}
 ...
 \label{eq:Qab}
\end{equation}
Let $\nu_{s,u}$, $0\le u\le1$, denote the last-spin cavity
interpolation obtained by replacing the interaction of the last spin with
the first $N-1$ spins by
\[
 \frac{\beta\sqrt{su}}{\sqrt N}
 \sum_{i<N}\frac{g_{iN}+g_{Ni}}{\sqrt2}\rho_i\varepsilon
 +\beta\sqrt{s(1-u)q}\,\widehat z\,\varepsilon,
\]
where $\widehat z$ is an independent standard Gaussian.  In particular,
$\nu_{s,1}=\nu_s$.

\begin{proposition}
\label{prop:cavity}
Let $\mathcal E=\binom{\mathbb N}{2}$ be the set of replica edges, fix
$e_\star=\{1,2\}$, and define the incidence map
\[
 \iota(e,e')=
 \begin{cases}
 A,&e=e',\\
 B,&|e\cap e'|=1,\\
 C,&e\cap e'=\varnothing.
 \end{cases}
\]
Let $H=H(\rho^1,\rho^2)$ be a bounded symmetric cavity observable,
$H(\rho^1,\rho^2)=H(\rho^2,\rho^1)$, independent of the last-spin
variables.  For
\[
 e_A=\{1,2\},\qquad e_B=\{1,3\},\qquad e_C=\{3,4\},
\]
write
$\varepsilon_{\{a,b\}}=\varepsilon_a\varepsilon_b$ and
$Q^-_{\{a,b\}}=Q^-_{ab}$, and define
\[
 g_E(u)=\nu_{s,u}[H(\varepsilon_{e_E}-q)],\qquad
 T_E=\nu_{s,0}[H Q^-_{e_E}],qquad E\in\{A,B,C\}.
\]
The decoupled covariance kernel depends only on $\iota(e,e')$:
\begin{equation}
 \mathbb E_0[(\varepsilon_e-q)\varepsilon_{e'}]
 =\theta_{\iota(e,e')},
 \qquad
...
\end{equation}
For $g=(g_A,g_B,g_C)^{\mathsf T}$ and
$T=(T_A,T_B,T_C)^{\mathsf T}$,
\begin{equation}
 g'(0)=\mathcal M_sT,
 \qquad
...
\end{equation}
Define the mode transformation $\Lambda\colon\mathbb R^3\to\mathbb R^3$ by
\[
 \Lambda=
 \begin{pmatrix}
 1&-4&3\\
 ...
 \Lambda T=
 \begin{pmatrix}
 \mathcal U(T)\\ \mathcal V(T)\\ \mathcal D(T)
 \end{pmatrix}.
\]
Then
\begin{equation}
 ...
 \begin{pmatrix}
 s\beta^2\kappa&0&0\\
 s\beta^2\zeta&s\beta^2\kappa&0\\
 0&0&s\alpha
 \end{pmatrix}\Lambda.
 \label{eq:cavity-general-modes}
\end{equation}
Let $\widehat T_N\in\mathbb R^3$ be an endpoint approximation to $T$.  If
\[
 \|T-\widehat T_N\|_\infty
 +\sup_{0\le u\le1}\|g''(u)\|_\infty=O(\eta_N),
\]
then
\[
 g(1)=g(0)+\mathcal M_s\widehat T_N+\mathcal R_N,
 \qquad
 \|\mathcal R_N\|_\infty=O(\eta_N).
\]
\end{proposition}

For $H=Q^-_{12}$, Proposition~\ref{prop:cavity}, the diagonal correction
in \eqref{eq:Qab}, and the endpoint replacement below give the scalar
specialization
\begin{align}
 ...
 \label{eq:cavity-D}
\end{align}
where, uniformly on $\cK\times[0,1]$,
\begin{equation}
...
\begin{proof}[Proof of Proposition~\ref{prop:cavity} and its scalar specialization]
All constants below depend only on $\cK$.  At $u=0$ the last spin is
independent of $\rho$ and has effective field
\[
 h+\beta\sqrt{(1-s)q}\,z_N+\beta\sqrt{sq}\,\widehat z
 \stackrel{\mathrm d}=h+\beta\sqrt q\,Z.
\]
Consequently, for distinct replica indices,
\begin{equation}
 \mathbb E_0[\varepsilon_a\varepsilon_b]=q,\qquad
...
\end{align}
Indeed, the derivative of the covariance of the interpolating last-spin
field is $s\beta^2\varepsilon\varepsilon'Q^-(\rho,\rho')$, and its
diagonal value is configuration-independent.

For $e,e'\in\mathcal E$, \eqref{eq:last-spin-moments-proof} gives
\begin{equation}
 \mathbb E_0[(\varepsilon_e-q)\varepsilon_{e'}]
 =
 \begin{cases}
 1-q^2,&e=e',\\
 q-q^2,&|e\cap e'|=1,\\
 r-q^2,&e\cap e'=\varnothing.
 \end{cases}
 \label{eq:edge-rule-proof}
\end{equation}
Apply \eqref{eq:cavity-derivative-proof} to
$F=H(\varepsilon_{e_E}-q)$, $E\in\{A,B,C\}$, and group each resulting
edge $e$ according to $\iota(e,e_\star)$.  Equation
\eqref{eq:edge-rule-proof} gives $g'(0)=\mathcal M_sT$.  Furthermore,
\[
 \theta_A-4\theta_B+3\theta_C=\kappa,\qquad
 2\theta_B-3\theta_C=\zeta,\qquad
 \theta_A-2\theta_B+\theta_C=\frac{\alpha}{\beta^2},
\]
and hence direct matrix multiplication gives
\eqref{eq:cavity-general-modes}.  Finally,
\[
 g(1)-g(0)-\mathcal M_s\widehat T_N
 =\mathcal M_s(T-\widehat T_N)
 +\int_0^1(1-u)g''(u)\,\dd u,
\]
which proves the general remainder statement.

For the scalar specialization, set
$M_3(u)\coloneqq\nu_{s,u}[|Q^-_{12}|^3]$,
\eqref{eq:cavity-derivative-proof} and $|Q^-_{ab}|\le2$ give
$|M_3'(u)|\le C_{\cK}M_3(u)$.  Since
$|Q^-_{12}|^3\le4|Q_{12}|^3+4N^{-3}$, backward Gronwall yields
\begin{equation}
 \sup_{0\le u\le1}\nu_{s,u}[|Q^-_{12}|^3]
 \le C_{\cK}\bigl(N^{-3/2}+\nu_s[|Q_{12}|^3]\bigr).
...
For
\[
 P^-=Q^-_{ab}Q^-_{cd},\qquad
 (ab,cd)\in\{(12,12),(12,13),(12,34)\},
\]
the derivative of $\nu_{s,u}[P^-]$ is a finite linear combination of
expectations of three cavity overlaps.  Thus, by H\"older's inequality,
\eqref{eq:M3-gronwall-proof}, \eqref{eq:Qab}, and
\[
...
after the disorder average gives
\[
 \begin{aligned}
 A_s&=\nu_s[Q_{12}(\varepsilon_1\varepsilon_2-q)],&
 B_s&=\nu_s[Q_{12}(\varepsilon_1\varepsilon_3-q)],&
 C_s&=\nu_s[Q_{12}(\varepsilon_3\varepsilon_4-q)].
 \end{aligned}
\]
Define
\[
 ...
 \end{aligned}
\]
and let $U_s^-,V_s^-,D_s^-$ be their modes.  Equation
\eqref{eq:endpoint-replacement-proof} implies
\begin{equation}
...
\[
 ...
 E_A=\varepsilon_1\varepsilon_2(\varepsilon_1\varepsilon_2-q),&
 E_B=\varepsilon_1\varepsilon_2(\varepsilon_1\varepsilon_3-q),&
 E_C=\varepsilon_1\varepsilon_2(\varepsilon_3\varepsilon_4-q).
 \end{array}
\]
Set $g_j(u)=\nu_{s,u}[G_j]$ and $e_j(u)=\nu_{s,u}[E_j]$ for
$j\in\{A,B,C\}$, and use the mode combinations in
\eqref{eq:cavity-modes} for $g_X,e_X$, $X\in\{U,V,D\}$.
Independence gives $g_j(0)=0$.  Applying
\eqref{eq:cavity-derivative-proof} twice, followed by
\eqref{eq:M3-gronwall-proof}, gives
\begin{equation}
 g_X(1)=g_X'(0)
 +O_{\cK}\bigl(N^{-3/2}+\nu_s[|Q_{12}|^3]\bigr),
 \qquad X\in\{U,V,D\}.
...
\end{equation}
Equations \eqref{eq:cavity-edge-matrix} and
\eqref{eq:cavity-general-modes} yield
\begin{align}
...
At the decoupled endpoint,
\begin{align}
 e_U(0)&=\kappa,\nonumber\\
 e_V(0)&=\zeta,
 \label{eq:scalar-diagonal-terms}\\
 e_D(0)&=\frac{\alpha}{\beta^2}.\nonumber
\end{align}
Since the $E_j$ are uniformly bounded,
\eqref{eq:cavity-derivative-proof} gives
$|e_X'(u)|\le C_{\cK}\nu_{s,u}[|Q^-_{12}|]$.  Hence
\eqref{eq:M3-gronwall-proof} and the preceding elementary inequality give
\begin{equation}
 \frac1N|e_X(1)-e_X(0)|
 \le C_{\cK}\bigl(N^{-3/2}+\nu_s[|Q_{12}|^3]\bigr),
 \qquad X\in\{U,V,D\}.
...
\end{equation}
Combining \eqref{eq:cavity-mode-replacement},
...
\eqref{eq:scalar-diagonal-remainders}, we obtain
\[
 \begin{aligned}
 ...
 \end{aligned}
\]
Absorbing these errors into
$\mathcal R^U_{N,s},\mathcal R^V_{N,s},\mathcal R^D_{N,s}$ proves
\eqref{eq:cavity-U}--\eqref{eq:cavity-remainders}.
\end{proof}

Since $r\le q$,
\[
 \alpha-\beta^2\kappa=2\beta^2(q-r)\ge0.
\]
Thus $\beta^2\kappa\le\alpha<1$.  If $\kappa<0$, then
$1-s\beta^2\kappa\ge1$, while if $\kappa\ge0$, then
$1-s\beta^2\kappa\ge1-\alpha$.  Hence \eqref{eq:delta} gives, uniformly
on $\cK\times[0,1]$,
\begin{equation}
 1-s\alpha\ge\delta_{\cK},\qquad
 1-s\beta^2\kappa\ge\delta_{\cK}.
 \label{eq:denominator-bounds}
\end{equation}
Solving \eqref{eq:cavity-U}--\eqref{eq:cavity-D} in the order
$U_s,V_s,D_s$ yields
\[
 |U_s|+|V_s|+|D_s|
 \le C_{\cK}\left(\frac1N+N^{-3/2}+\nu_s[|Q_{12}|^3]\right).
\]
Since $A_s=-V_s-2U_s+3D_s$, it follows that
\begin{equation}
 A_s\le\frac{C_{\cK}}N+C_{\cK}\nu_s[|Q_{12}|^3].
 \label{eq:preabsorb}
\end{equation}


\section{Quantitative bound for overlaps, free energy, and susceptibility}\label{sec:conclusion}
...
We follow the same argument as in \cite[Chapter 1]{Talagrand}. In this section we work at $s=1$ and suppress the subscript $s$.  Put
\[
 a\coloneqq1-2q+r=\frac{\alpha}{\beta^2}.
\]
The argument above \eqref{eq:denominator-bounds} gives
...
\begin{proof}[Proof of Theorem~\ref{thm:overlap-clt-intro}]
Let $\nu_u$, $0\le u\le1$, be the last-spin interpolation from
Appendix~\ref{app:cavity-identities}.  Thus $\nu_1=\nu$, and
\[
 \varepsilon_a=\sigma_N^a,\qquad
 Q^-_{ab}=\frac1N\sum_{i<N}\sigma_i^a\sigma_i^b-q,\qquad
 Q_{ab}=Q^-_{ab}+\frac1N\varepsilon_a\varepsilon_b.
\]
The overlap estimate proved above gives
\begin{equation}
 \nu[Q_{12}^2]\le\frac{C}{N}.
 \label{eq:clt-endpoint-second-moment}
\end{equation}
For $M_2(u)=\nu_u[(Q^-_{12})^2]$,
\eqref{eq:cavity-derivative-proof} and $|Q^-_{ab}|\le2$ imply
\begin{equation}
 |M_2'(u)|\le C M_2(u).
 \label{eq:clt-M2-differential}
\end{equation}
Since $M_2(1)\le2\nu[Q_{12}^2]+2N^{-2}$, backward Gronwall yields
\begin{equation}
 \sup_{0\le u\le1}\nu_u[(Q^-_{12})^2]\le\frac{C}{N}.
 \label{eq:clt-uniform-cavity-second}
\end{equation}
Replica symmetry and Cauchy--Schwarz therefore give, for fixed edges
$ab,cd$,
\begin{align}
 \sup_{0\le u\le1}\nu_u[|Q^-_{ab}|]
 \le\frac{C}{\sqrt N},
\quad  \sup_{0\le u\le1}\nu_u[|Q^-_{ab}Q^-_{cd}|]
 \le\frac{C}{N}.
 \label{eq:clt-cavity-product}
\end{align}

Fix $f\in C^2(\mathbb R)$ with bounded $f,f',f''$, and set
$X_{ab}=\sqrt N\,Q_{ab}$.  Define
\begin{align}
 T_A(f)&\coloneqq\nu[X_{12}f(X_{12})],
 \label{eq:clt-TA}\\
 T_B(f)&\coloneqq\nu[X_{13}f(X_{12})],
 ...
 \label{eq:clt-TC}
\end{align}
Site exchangeability gives
\begin{equation}
 T_E(f)=\sqrt N\,\nu[
 \overline\varepsilon_{e(E)}f(X_{12})],
 \quad
 \overline\varepsilon_{ab}\coloneqq\varepsilon_a\varepsilon_b-q,
 \quad
 e(A)=12,\ e(B)=13,\ e(C)=34.
 \label{eq:clt-site-exchangeability}
\end{equation}
Put
\[
 g_E(u)\coloneqq
 \sqrt N\,\nu_u[\overline\varepsilon_{e(E)}f(X_{12})],
 \qquad E\in\{A,B,C\}.
\]
Then $g_E(1)=T_E(f)$.  We write $O_f(\cdot)$ for a bound whose constant
depends only on $\beta,h$ and
$\|f\|_\infty+\|f'\|_\infty+\|f''\|_\infty$.

Remove the last-spin contribution from the argument of $f$ by setting
\begin{equation}
 Y\coloneqq\sqrt N\,Q^-_{12}+\frac q{\sqrt N}.
 \label{eq:clt-Yminus}
\end{equation}
Then
\begin{equation}
 X_{12}=Y+\frac{\overline\varepsilon_{12}}{\sqrt N}.
 \label{eq:clt-X-Y}
\end{equation}
At $u=0$, \eqref{eq:last-spin-moments-proof} gives
\begin{align}
 \mathbb E_0[\overline\varepsilon_{12}^2]
 &=1-q^2,
 \label{eq:clt-source-A}\\
 \mathbb E_0[\overline\varepsilon_{13}\overline\varepsilon_{12}]
 &=q-q^2,
 \label{eq:clt-source-B}\\
 \mathbb E_0[\overline\varepsilon_{34}\overline\varepsilon_{12}]
 &=r-q^2.
 \label{eq:clt-source-C}
\end{align}
Taylor expansion in \eqref{eq:clt-X-Y} and independence at $u=0$ imply
\begin{equation}
 g_E(0)=\theta_E\nu_0[f'(Y)]+O_f(N^{-1/2}),
...
\begin{equation}
 ...
 \label{eq:clt-theta}
\end{equation}
By \eqref{eq:cavity-derivative-proof} and
\eqref{eq:clt-cavity-product},
\[
 \left|\frac{\mathrm d}{\mathrm du}\nu_u[f'(Y)]\right|
 \le\frac{C\|f'\|_\infty}{\sqrt N}.
\]
Together with \eqref{eq:clt-X-Y}, this gives the reusable endpoint
replacement
\begin{equation}
 \nu_0[f'(Y)]=\nu[f'(X_{12})]+O_f(N^{-1/2}).
...
\end{align}
In $g_E'(0)$, we can replace $f(X_{12})$ by $f(Y)$ with error
$O_f(N^{-1/2})$ due to \eqref{eq:clt-X-Y} and
\eqref{eq:clt-cavity-product}.  Thus the three-edge calculation in
\eqref{eq:cavity-edge-matrix}, specialized to $s=1$, gives
\begin{align}
 g_A'(0)&=\beta^2\bigl(
 \theta_A\widetilde T_A-4\theta_B\widetilde T_B
 +3\theta_C\widetilde T_C\bigr)+O_f(N^{-1/2}),
 \label{eq:clt-row-A}\\
 g_B'(0)&=\beta^2\bigl(
 \theta_B\widetilde T_A
 +(\theta_A-2\theta_B-3\theta_C)\widetilde T_B
 +(6\theta_C-3\theta_B)\widetilde T_C\bigr)
 +O_f(N^{-1/2}),
 \label{eq:clt-row-B}\\
 g_C'(0)&=\beta^2\bigl(
 \theta_C\widetilde T_A
 +(4\theta_B-8\theta_C)\widetilde T_B
 +(\theta_A-8\theta_B+10\theta_C)\widetilde T_C\bigr)
 +O_f(N^{-1/2}).
 \label{eq:clt-row-C}
\end{align}
Equivalently, for $T=(T_A,T_B,T_C)^{\mathsf T}$,
$\widetilde T=(\widetilde T_A,\widetilde T_B,\widetilde T_C)^{\mathsf T}$,
and $\theta=(\theta_A,\theta_B,\theta_C)^{\mathsf T}$, let
\begin{equation}
 M\coloneqq\mathcal M_1
 =\beta^2
 \begin{pmatrix}
 \theta_A&-4\theta_B&3\theta_C\\
 \theta_B&\theta_A-2\theta_B-3\theta_C&6\theta_C-3\theta_B\\
...
Then
\begin{equation}
 g'(0)=M\widetilde T(f)+O_f(N^{-1/2}).
 \label{eq:clt-g-prime-zero}
\end{equation}

A second application of \eqref{eq:cavity-derivative-proof} and
\eqref{eq:clt-cavity-product} gives
\begin{equation}
 \sup_{0\le u\le1}|g_E''(u)|\le\frac{C_f}{\sqrt N}.
 \label{eq:clt-second-cavity-derivative}
\end{equation}
Therefore
\begin{equation}
 T_E(f)=g_E(0)+g_E'(0)+O_f(N^{-1/2}).
 \label{eq:clt-cavity-Taylor}
\end{equation}
Similarly, differentiating
$\sqrt N\,\nu_u[Q^-_{e(E)}f(Y)]$ and using
\eqref{eq:clt-cavity-product} gives
\begin{equation}
 \sqrt N\,\nu_0[Q^-_{e(E)}f(Y)]
 =
 \sqrt N\,\nu[Q^-_{e(E)}f(Y)]+O_f(N^{-1/2}).
 \label{eq:clt-u-endpoint-replacement}
\end{equation}
Since $Q_{e(E)}-Q^-_{e(E)}=N^{-1}\varepsilon_{e(E)}$, equations
\eqref{eq:clt-X-Y} and \eqref{eq:clt-cavity-product} further imply
\begin{equation}
 \widetilde T_E(f)=T_E(f)+O_f(N^{-1/2}),
 \qquad E\in\{A,B,C\}.
 \label{eq:clt-Ttilde-T}
\end{equation}
Combining \eqref{eq:clt-g-zero}, \eqref{eq:clt-g-prime-zero},
\eqref{eq:clt-cavity-Taylor}, and \eqref{eq:clt-Ttilde-T}, we obtain
\begin{equation}
 T(f)=\theta m_f+MT(f)+O_f(N^{-1/2}),
 \qquad m_f=\nu[f'(X_{12})].
 \label{eq:clt-vector-Stein}
\end{equation}

Define the mode correlations
\begin{align}
 ...
 \label{eq:clt-Df}
\end{align}
The source modes are $\kappa,\zeta,a$, respectively.  Applying
\eqref{eq:cavity-general-modes} at $s=1$ to
\eqref{eq:clt-vector-Stein} gives
\begin{align}
 (1-\beta^2\kappa)U_f
 &=\kappa m_f+O_f(N^{-1/2}),
...
\end{align}
By \eqref{eq:clt-positive-denominators},
\begin{align}
 U_f&=\frac{\kappa}{1-\beta^2\kappa}m_f+O_f(N^{-1/2}),
 \label{eq:clt-U-solved}\\
 D_f&=\frac{a}{1-\alpha}m_f+O_f(N^{-1/2}).
 \label{eq:clt-D-solved}
\end{align}
Substitution in \eqref{eq:clt-Stein-V} yields
\begin{equation}
 V_f=\frac{\zeta}{(1-\beta^2\kappa)^2}m_f+O_f(N^{-1/2}).
 \label{eq:clt-V-solved}
\end{equation}
The inverse mode transformation gives
\begin{equation}
 T_A(f)=-V_f-2U_f+3D_f.
...
\end{align}
Taking $f\equiv1$ gives
\begin{equation}
 \nu[X_{12}]=O(N^{-1/2}).
 \label{eq:clt-bias}
\end{equation}

We next verify that $\sigma^2\ge0$.  The estimate
$\nu[|Q_{12}|^3]=o(N^{-1})$ and
\eqref{eq:cavity-remainders} show that the remainders in
\eqref{eq:cavity-U}--\eqref{eq:cavity-D} are $o(N^{-1})$ at $s=1$.
Thus
\begin{equation}
 NU\longrightarrow\frac{\kappa}{1-\beta^2\kappa}.
 \label{eq:clt-second-U}
\end{equation}
The equations for $V$ and $D$ then give
\begin{align}
 NV&\longrightarrow\frac{\zeta}{(1-\beta^2\kappa)^2},
 \label{eq:clt-second-V}\\
 ND&\longrightarrow\frac{a}{1-\alpha}.
 \label{eq:clt-second-D}
\end{align}
Since $A=-V-2U+3D$,
\begin{equation}
 N\nu[Q_{12}^2]=NA\longrightarrow
 \frac{3a}{1-\alpha}
 -\frac{2\kappa}{1-\beta^2\kappa}
 -\frac{\zeta}{(1-\beta^2\kappa)^2}
 =\sigma^2.
...
\end{align}
For fixed $T>0$, the $O_f(N^{-1/2})$ error in
\eqref{eq:clt-scalar-Stein} is uniform for
$f(x)=\sin(tx),\cos(tx)$ and $|t|\le T$.  Therefore
\begin{align}
 C_N'(t)+\sigma^2tC_N(t)
 &=O_T(N^{-1/2}),
 \qquad |t|\le T,
...
\end{align}
Using the integrating factor $e^{\sigma^2t^2/2}$ and
$C_N(0)=1$, $S_N(0)=0$, we obtain
\begin{equation}
 \sup_{|t|\le T}
 \left|C_N(t)-e^{-\sigma^2t^2/2}\right|
 \longrightarrow0,
 \label{eq:clt-C-uniform}
\end{equation}
and
\begin{equation}
 \sup_{|t|\le T}|S_N(t)|\longrightarrow0.
 \label{eq:clt-S-uniform}
\end{equation}
Since $T$ is arbitrary, using the Euler identity $e^{ix} = \cos{x} + i \sin{x}$,  these limits prove
\[
 \nu[e^{itX_{12}}]\longrightarrow e^{-\sigma^2t^2/2}.
\]
By \eqref{eq:clt-variance-nonnegative}, the limit is the characteristic
function of $\mathcal N(0,\sigma^2)$. By a standard result in probability theory, we have  
\[
 \sqrt N\,(R_{12}-q)\xrightarrow{\mathrm d}\mathcal N(0,\sigma^2).
\]
\end{proof}
...
Then, for any $f\in C_b^2 ({\mathbb R}^n)$, it holds that\footnote{\textcolor{red}{The statement permits singular covariance matrices, but the proof below uses $\det C(t)$ and $C(t)^{-1}$.  As written, that proof covers only positive-definite $C(t)$.  The singular case requires an approximation $C\mapsto C+\varepsilon I$ followed by $\varepsilon\downarrow0$, or a proof using characteristic functions.}}
\[
...
\bibitem{CometsNeveu}
F.~Comets and J.~Neveu,
\newblock
\emph{The Sherrington--Kirkpatrick model of spin glasses and stochastic
calculus: the high temperature case},
\newblock \emph{Communications in Mathematical Physics} 166 (1995), no.~3,
549--564.
...
\bibitem{DuHuangCriticalOverlap}
H.~Du and B.~Huang,
\newblock
\emph{Overlap distribution of the critical Sherrington--Kirkpatrick model},
\newblock preprint (2026).
\href{https://arxiv.org/abs/2608.08752}{arXiv:2608.08752}.
...
\bibitem{GuerraToninelliCLT}
F.~Guerra and F.~L.~Toninelli,
\newblock
\emph{Central limit theorem for fluctuations in the high temperature region
of the Sherrington--Kirkpatrick spin glass model},
\newblock \emph{Journal of Mathematical Physics} 43 (2002), no.~12,
6224--6237.
\href{https://arxiv.org/abs/cond-mat/0201092}{arXiv:cond-mat/0201092}.
...
\bibitem{TalagrandRSB}
M.~Talagrand,
\newblock
\emph{Replica symmetry breaking and exponential inequalities for the
Sherrington--Kirkpatrick model},
\newblock \emph{Annals of Probability} 28 (2000), no.~3, 1018--1062.
