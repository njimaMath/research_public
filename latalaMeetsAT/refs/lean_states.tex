...
\begin{center}
  ...
  Snapshot checked on 2 August 2026
\end{center}
...
The directory \lean{Lemmas} contains 24 Lean files.  Seventeen files have
no local \lean{sorry}; seven files contain seven occurrences in total.  There are
...
differentiation, GT interpolation, GT functional coercivity, the last-spin
construction, the cavity-remainder estimate, and the free-energy endpoint
facts are now isolated as ordinary theorems with localized placeholders,
except that the current cavity-interpolation existence statement is discharged
by a zero witness because it does not encode the intended endpoint identities.
...
\lean{GT/Coercivity.lean} & \openstate{1} & The local constant, abstract Taylor-loss lemma, overlap-range lemma, and composition theorem are proved.  The analytic estimate \lean{gtFunctional_coercivity} remains. \\
...
GT coercivity theorem & difficult & Codex & Proving \lean{gtFunctional_coercivity} requires multiplier derivatives, a uniform second-derivative estimate, the scalar linear bounds, and signed far-from-$q$ estimates. \\
...
Top-level certification & moderate & Codex & The assembly and bridge-free endpoint wrapper build.  Certification requires replacing the seven theorem placeholders, repairing the cavity interpolation specification, and auditing the resulting axiom set. \\
\bottomrule
\end{longtable}
\normalsize

\section{The seven remaining sorry obligations}
...
\item \lean{GT/Coercivity.lean:gtFunctional_coercivity}.  Prove multiplier
differentiability, uniform Taylor control, and the signed estimates away from
$q$.
...
These interfaces make dependencies visible, but their placeholder existence
theorems must be proved before certification.
...
then prove \lean{gtFunctional_coercivity}.  Expose multiplier derivatives and
factorization as reusable named lemmas.
...
\lean{GT/Coercivity.lean:gtFunctional_coercivity} are localized placeholders.
The explicit recursion and the composition theorem
\lean{gt_quadratic_coercivity} are present, but neither core analytic estimate
has been proved.
